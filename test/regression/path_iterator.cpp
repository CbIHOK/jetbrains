...
TYPED_TEST( path_iterator_test, valid_path )
{
    ...
    EXPECT_NO_THROW( auto head = path_begin( path_1 ); EXPECT_TRUE( head.is_valid() ); EXPECT_NE( head, path_iterator< test_type >() ) );
    EXPECT_NO_THROW( auto tile = path_end( path_1 ); EXPECT_TRUE( tile.is_valid() ); EXPECT_NE( tile, path_iterator< test_type >() ) );
