...
namespace jb
{
    ...
    namespace details
    {
        ...
        {
            template < typename Policies, typename TestHooks > friend class Storage;


            using self_type = virtual_volume< Policies >;
            ...
            using MountPoint = std::conditional_t< std::is_void_v< TestHooks >, mount_point< Policies >, typename TestHooks::MountPointT >;
            using MountPointPtr = std::shared_ptr< MountPoint >;
            using PhysicalVolume = std::conditional_t< std::is_void_v< TestHooks >, physical_volume< Policies >, typename TestHooks::PhysicalVolumeT >;
            ...
            struct tag_logical_path {};
            using ndx_logical_path = bmi::hashed_non_unique< bmi::tag< tag_logical_path >, bmi::const_mem_fun< MountPoint, KeyView, &MountPoint::logical_path > >;

            struct tag_priority {};
            using ndx_priority = bmi::ordered_unique< bmi::tag< tag_priority >, bmi::identity< MountPointPtr >, priority_compare >;

            struct tag_children {};
            using ndx_children = bmi::hashed_non_unique< bmi::tag< tag_children >, bmi::const_mem_fun< MountPoint, MountPointPtr, &MountPoint::parent > >;

            struct tag_uniqueness {};
            using ndx_uniqueness = bmi::hashed_unique<
                bmi::tag< tag_uniqueness >, 
                bmi::composite_key< 
                    MountPointPtr, 
                    bmi::const_mem_fun< MountPoint, KeyView, &MountPoint::logical_path >, 
                    bmi::mem_fun< MountPointPtr, MountPoint*, &MountPointPtr::get >
                >
            >;

            using mount_collection_t = boost::multi_index_container< MountPointPtr, bmi::indexed_by< ndx_logical_path, ndx_priority, ndx_children, ndx_uniqueness > >;
            ...
            */
            ...
            {
                const auto & by_logical_path = mounts_.get< tag_logical_path >();
            ...
            */
            virtual_volume() _NOEXCEPT {}


            /** The class is not copyable/movable
            */
            virtual_volume( self_type && ) = delete;
        ...
        public:
            ...
            */
            [ [ nodiscard ] ]
            std::tuple< RetCode, std::weak_ptr< MountPoint > > mount(   std::weak_ptr< PhysicalVolume > physical_volume,
                                                                        ...
                                                                        Key && logical_path ) _NOEXCEPT
            {
                try
                {
                    ...
                    {
                        return { InvalidPhysicalVolume, std::weak_ptr< MountPoint >{} };
                    }
                    else if ( !detail::is_valid_path( physical_path ) )
                    {
                        return { InvalidPhysicalPath, std::weak_ptr< MountPoint >{} };
                    }
                    else if ( !detail::is_valid_path( logical_path ) )
                    {
                        return { InvalidLogicalPath, std::weak_ptr< MountPoint >{} };
                    }
                    ...
                    {
                        return { Ok, std::weak_ptr< MountPoint >{ mount_point } };
                    }
                }
                catch ( const bad_alloc & )
                {
                    return { RetCode::InsufficientMemory, std::weak_ptr< MountPoint >{} };
                }
                catch ( ... )
                {
                    return { RetCode::UnknownError, std::weak_ptr< MountPoint >{} }
                }
            }
